...
impl<'a> Aggregator<'a> {
    ...
    pub fn compute_interaction_type_aggregation(
        ...
        let filtered: Vec<_> = if let Some(tid) = team_id {
            // Get team member user_ids
            let members = self.db.get_team_members(organization_id, tid)?;
            let member_ids: Vec<_> = members.iter().map(|m| m.user_id.as_str()).collect();
            classifications
                .iter()
                .filter(|c| member_ids.contains(&c.sender_id.as_str()))
                .collect()
        } else {
    ...
    pub fn compute_sentiment_aggregation(
        ...
        // Rolling averages (computed from DB if available)
        let rolling_7day_avg = self.compute_rolling_sentiment_avg(organization_id, team_id, 7)?;
        let rolling_30day_avg = self.compute_rolling_sentiment_avg(organization_id, team_id, 30)?;
    ...
    pub fn run_all_team_aggregations(
        ...
        // First, run org-level aggregation
        let org_result = self.run_aggregations(
            organization_id,
            None,
            period_start,
            period_end,
        ...
        for team in &teams {
            let result = self.run_aggregations(
                organization_id,
                Some(&team.team_id),
                period_start,
                period_end,
    ...
    // Helper: compute rolling sentiment average
    fn compute_rolling_sentiment_avg(
        ...
        let classifications = self.db.get_classifications_in_range(organization_id, &start, &end)?;
