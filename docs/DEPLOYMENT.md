...
```bash
cd /path/to/fd-ruvector-marshal
...
```bash
gcloud storage buckets create gs://goalign-rag-bucket \
...
# Grant permissions
gcloud projects add-iam-policy-binding goalign-alpha \
  --member="serviceAccount:rag-service@goalign-alpha.iam.gserviceaccount.com" \
...
gcloud iam service-accounts keys create rag-key.json \
  --iam-account=rag-service@goalign-alpha.iam.gserviceaccount.com
```
...
```bash
docker build -t gcr.io/goalign-alpha/ruvector-streaming:latest \
  -f src/cloud-run/Dockerfile .

docker push gcr.io/goalign-alpha/ruvector-streaming:latest
```
...
gcloud run deploy ruvector-rag \
  --image=gcr.io/goalign-alpha/ruvector-streaming:latest \
  --region=us-central1 \
  --platform=managed \
